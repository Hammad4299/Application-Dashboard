...
Object.defineProperty(exports, "__esModule", { value: true });
var ReportingApiHelper_1 = __webpack_require__(23);
...
var MoneyMakerPiwikHelper = function (_ReportingApiHelper_) {
    ...
    _createClass(MoneyMakerPiwikHelper, [{
        ...
        value: function getUserAnalyticDetail(userId, startDate, endDate, callback) {
            ...
            var self = this;
            this.getVisitor(userId, function (visitor) {
                var visitorId = self.getVisitorId(visitor);
                if (visitorId === null) {
                    callback(null);
                } else {
                    var r = new ReportingApiHelper_1.VisitorProfileRequest(_this2.config);
                    r.setVisitorId(visitorId);
                    r.setDateRange(startDate, endDate);
                    console.log(r);
                    _this2.executeRequest(r, function (d) {
...
var globals_1 = __webpack_require__(0);
var MoneyMakerPiwikHelper_1 = __webpack_require__(24);
var time_1 = __webpack_require__(3);
var uid;
var email;
var username;
...
$(document).on('click', '.js-analytics-by-date', function () {
    var startdate = $(this).parent().find('.date').val().split(' to ')[0];
    var enddate = $(this).parent().find('.date').val().split(' to ')[1];
    ...
    piwikHelper.getUserAnalyticDetail(uid, startdate, enddate, function (data) {
        ...
        visitedScreens.map(function (screen) {
            ...
            toIns[1] = globals_1.default.timeHelper.convertUtcToUserTime(screen.timestamp.toString(), "X").format('YYYY-MM-DD').toString();
            toIns[2] = (parseInt(toIns[2]) + screen.duration).toString();
            map[screen.actionName] = toIns;
        });
        for (var i = 0; i < flattendEvents.length; i++) {
            eventDataSet.push([flattendEvents[i].actionName != undefined ? flattendEvents[i].actionName : " ", flattendEvents[i].actionPerformed != undefined ? flattendEvents[i].actionPerformed : " ", flattendEvents[i].eventCategory != undefined ? flattendEvents[i].eventCategory : " ", flattendEvents[i].timestamp != undefined ? globals_1.default.timeHelper.convertUtcToUserTime(flattendEvents[i].timestamp, "X").format('YYYY-MM-DD') : " "]);
        }
        ...
        $('#screens').append('<table class="display" style="width: 100%"></table>').find('table').DataTable({
            ...
            columns: [{ title: "Screen Name" }, { title: "Visited At" }, { title: "Duration" }]
        });
        ...
        $('#events').append('<table class="display" style="width: 100%"></table>').find('table').DataTable({
            ...
            searching: false,
            lengthMenu: [[5, 25, 50, -1], [5, 25, 50, "All"]],
            columns: [{ title: "Action Name" }, { title: "Action Performed" }, { title: "Event Category" }, { title: "Timestamp" }]
        });
    });
});
$(document).on('click', '.js-analytics-detail', function () {
    ...
    piwikHelper.getUserAnalyticDetail(uid, null, null, function (data) {
        console.log(email);
        console.log(username);
        console.log(data.getFlattenedEvents());
        console.log(data.getVisitScreens());
